...
export class PackagingScanComponent implements OnInit, AfterViewInit {
  ...
  packagingBox: BehaviorSubject<PackagingBoxDto> =
    new BehaviorSubject<PackagingBoxDto>({
      id: 0,
      barcode: '',
      created_by: '1',
      harness_id: 0,
  ...
  packagingProcess: BehaviorSubject<PackagingProcess> =
    new BehaviorSubject<PackagingProcess>({
      id: 1,
      segmentId: 1,
      segment: null,
      status: 1,
  ...
  packagingSteps: BehaviorSubject<PackagingStep[]> = new BehaviorSubject<
    PackagingStep[]
  >([]);
  packagingStepsAll: BehaviorSubject<PackagingStep[]> = new BehaviorSubject<
    PackagingStep[]
  >([]);
  loopsSteps: BehaviorSubject<PackagingStep[]> = new BehaviorSubject<
    PackagingStep[]
  >([]);
  ...
  openedBoxes: BehaviorSubject<PackagingBoxDto[]> = new BehaviorSubject<
    PackagingBoxDto[]
  >([]);
  ...
  ngOnInit(): void {
      ...
      ?.valueChanges.pipe(
        ...
        tap((value) => {
          if (
            value != '' &&
            value ===
              this.storageService.getItem('current_box_prefix') +
                this.packagingBox.getValue().barcode
          ) {
            this.stepper.reset();
    ...
    if (isNaN(selectedPackagingProcess)) {
      ...
      this.packagingProcessService
        .getProcessById(selectedPackagingProcess)
        .pipe(
          tap((packagingProcess: PackagingProcess) => {
            let packagingStepsAll = packagingProcess.steps.sort(
              (a, b) => a.order - b.order
            );
            let packagingSteps = packagingProcess.steps
              .filter((step) => step.name.includes('packaging'))
              .sort((a, b) => a.order - b.order);
            let loopSteps = packagingProcess.steps
              .filter((step) => !step.name.includes('packaging'))
              .sort((a, b) => a.order - b.order);
    ...
    this.packagingBoxService
      .getOpendPackageByLineId(lineId)
      .pipe(
        tap((packageBoxs: PackagingBoxDto[]) => {
          ...
          if (currentBox) {
            ...
            this.snackBar.open(
              'Please complete the following package ' + currentBox.barcode,
              'ok',
              {
                duration: 50000,
            ...
            this.currentRef.next(
              currentBox.harness ? currentBox.harness.ref : ''
            );
            // navigate to step
            for (
              let index = 0;
              index < this.packagingSteps.getValue().length + 1;
              index++
            ) {
              this.stepper.next();
            }
  ...
  async evaluateStepCondition(step: PackagingStep): Promise<void> {
    ...
    if (conditionMet) {
      ...
      const totalQuantity =
        this.packagingBox.getValue().to_be_delivered_quantity;
      if (step.order === totalSteps) {
        let prodHarness = new CreateProdHarnessDTO(
          this.currentCounter.getValue(),
          null,
          this.packagingBox.getValue().barcode,
          null,
          this.packagingBox.getValue().id,
          2,
          this.packagingBox.getValue().harness.id
        );
        this.prodHarnessService
          .createProdHarness(prodHarness)
          .pipe(
            tap((response) => {
              if (response && response.response) {
                ...
                this.snackBar.open(
                  'Failed to create new production harness',
                  'ok',
                  { duration: 3000 }
                );
                this.activeStep.next(this.loopsSteps.getValue()[0].order);
            ...
            catchError((error) => {
              console.error(
                'Error occurred while creating ProdHarness:',
                error
              );
              this.snackBar.open(
                'Failed to create new production harness',
  ...
  initializeCharts(): void {
    ...
    this.countFxPerHour.map((value) => {
      this.totalOfDeliveredHarnessPerShift += parseInt(
        value.total_quantity.toString()
      );
    });
   ...
   * @param event
   */
  @HostListener('window:keydown.enter', ['$event']) nextStepEnter(
    event: KeyboardEvent
  ): void {
  ...
  focusCtrl(stepNumber: number): void {
    setTimeout(() => {
      const CtrlInput = document.getElementById(
        `step${stepNumber}`
      ) as HTMLInputElement;
      if (CtrlInput) {
        CtrlInput.focus();
                ...
                this.packagingBoxService.checkIfBoxExsit(partNumber).subscribe({
                  next: (value) => {
                    if (!value.status) {
                      ...
                      this.snackBar.open('Box already exist ', 'Ok', {
                        duration: 5000,
                      });
                      resolve(false);
                    }
        ...
        case 'packaging#Box harness-reference':
          ...
          if (step.pre_fix != '') {
            ...
            if (preFix === step.pre_fix) {
              ...
              let harnessReference = input.substring(
                preFix.length,
                input.length
              );
                ...
                this.checkIfCounterExist(counter).then((exist) => {
                  if (exist) {
                    ...
                    if (this.currentCounter.getValue() === '') {
                      ...
                      this.currentCounter.getValue() === counter
                        ? resolve(true)
                        : resolve(false);
                    }
                  }
          ...
          if (formValue['BOL 1#Harness reference'] !== '') {
            ...
            if (step.pre_fix != '') {
              ...
              if (preFix === step.pre_fix) {
                ...
                // check equality
                if (
                  harnessReference == this.currentRef.getValue() ||
                  harnessReference == this.packagingBox.getValue().harness.ref
                ) {
                  resolve(true);
                } else {
                  resolve(false);
              ...
              if (
                input == this.currentRef.getValue() ||
                input == this.packagingBox.getValue().harness.ref
              ) {
                resolve(true);
              } else {
                resolve(false);
  ...
  createPackage(packagingBoxDto: PackagingBoxDto): void {
    this.packagingBoxService
      .createPackagingBox(packagingBoxDto)
      .pipe(
        tap((packagingBox) => {
  ...
  async checkIfCounterExist(counter: string): Promise<boolean> {
    try {
      const responseData = await firstValueFrom(
        this.prodHarnessService.getByRef(counter)
      );
  ...
  changeSelectedBox(id: number) {
    this.packagingBoxService
      ...
      .pipe(
        ...
        catchError((err) => {
          this.snackBar.open(
            `Error during select package as default: ${err.message}`
          );
          return of(null);
        })
      )
  ...
  pauseCurrent() {
    const box = this.packagingBox.getValue();
    box.status = 0;
    this.packagingBoxService.updatePackagingBox(box.id, box).subscribe((_) => {
      window.location.reload();
    });
  }
}
