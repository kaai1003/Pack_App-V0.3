...
export class PackagingSettingsComponent implements OnInit, OnDestroy, AfterViewInit {
  ...
  constructor(private printingService: PrintingService, private fromBuilder: FormBuilder, private productionLineService: ProductionLineService, private segmentService: SegmentService, private snakeBar: MatSnackBar, private packagingProcessService: PackagingProcessService, private storageService: StorageService, private router: Router, private projectService: ProjectService) {
  ...
  ngOnInit(): void {
    ...
    this.lineSettings.get('line')?.valueChanges.pipe(tap(value => {
    ...
    this.printingService.getAllPrinters().pipe(tap(value => {
    ...
    this.printresFrom.get('default_printer')?.valueChanges.subscribe(selectedPrinter => {
      this.printingService.setDefaultPrinter(new SetDefaultPrinterRequest(selectedPrinter)).subscribe(response => {
    ...
    this.lineSettings.get("process")?.valueChanges.pipe(tap(value => {
    ...
    this.segmentService.getAllSegment().pipe(tap(value => {
    ...
    this.packagingProcessService.getAllProcesses().pipe(tap(packagingProcess => {
  ...
  ngAfterViewInit(): void {
    ...
    this.productionLineService.getAll().pipe(tap((value) => {
